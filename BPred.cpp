...
BPred *BPredictor::getBPred(int32_t id, int32_t fetchWidth, const char *sec)
{
    ...
    } else if (strcasecmp(type, "Hier") == 0) {
        pred = new BPHier(id, fetchWidth, sec);
    } 
    else {
...
BPHier::BPHier(int32_t i, int32_t fetchWidth, const char *section)
    :BPred(i, fetchWidth, section,"Hier")
    ,btb(  i, fetchWidth, section)
    ,historySize(SescConf->getInt(section,"historySize"))
    ,historyMask((1 << historySize) - 1)
    ,globalTable(i,section
...
PredType BPHier::predict(const Instruction *inst, InstID oracleID, bool doUpdate)
{
    bpredEnergy->inc();
    ...
    // update historyTable statistics
    if (doUpdate) {
        ghr = ((ghr << 1) | ((iID>>2 & 1)^(taken?1:0)));
    }

    //Consult localTable first
    if(doUpdate) {
        localTaken = localTable.predict(iID, taken);
    }
    else {
    ...
    if (localTable.isLowest(iID) || localTable.isHighest(iID)) { //Fully Saturated

        ptaken = localTaken;
    }

    else { //Not saturated, so use a more sophisticated predictor
        if(doUpdate) {
            globalTaken = globalTable.predict(l2Index, taken);
        }
        else {
...
void BPHier::switchIn(Pid_t pid)
{
}

void BPHier::switchOut(Pid_t pid)
{
}